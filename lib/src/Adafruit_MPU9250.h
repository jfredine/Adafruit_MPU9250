...
#define MPU9250_AK8963_ASAX \
    0x10  ///< Fuse ROM x-axis sensitivity adjustment value
#define MPU9250_AK8963_ASAY \
    0x11  ///< Fuse ROM y-axis sensitivity adjustment value
#define MPU9250_AK8963_ASAZ \
    0x12  ///< Fuse ROM z-axis sensitivity adjustment value
...
class Adafruit_MPU9250_Magnetometer : public Adafruit_Sensor {
   public:
    /** @brief Create an Adafruit_Sensor compatible object for the magnetometer
        @param parent A pointer to the MPU9250 class */
    Adafruit_MPU9250_Magnetometer(Adafruit_MPU9250 *parent) {
        _theMPU9250 = parent;
    }
    bool getEvent(sensors_event_t *);
    void getSensor(sensor_t *);
...
class Adafruit_MPU9250 final {
   public:
    ...
    int begin(uint8_t i2c_addr = MPU9250_I2CADDR_DEFAULT, TwoWire *wire = &Wire,
              int32_t sensorID = 0);
    ...
    bool setGyroStandby(bool xAxisStandby, bool yAxisStandby,
                        bool zAxisStandby);
   ...
   protected:
    ...
    Adafruit_I2CDevice *ak8963_i2c_dev =
        NULL;  ///< Pointer to I2C bus interface
    ...
    Adafruit_MPU9250_Magnetometer *mag_sensor =
        NULL;  ///< Magnetometer data object
   ...
   private:
    ...
    friend class Adafruit_MPU9250_Magnetometer;  ///< Gives access to private
                                                 ///< members to Mag data object
